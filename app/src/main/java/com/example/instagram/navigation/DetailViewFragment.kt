...
class DetailViewFragment : Fragment() {
    ...
    inner class DetailViewRecyclerViewAdater : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
        ...
        init {
            ...
            firestore?.collection("images")?.orderBy("timestamp")?.addSnapshotListener { querySnapshot, firebaseFirestoreExcept ->
                contentDTOs.clear()
        ...
        fun favoriteEvent(position: Int){
            ...
            firestore?.runTransaction { transaction ->
                ...
                // 이미 favorite를 누른 uid인지 아닌지
                if(contentDTO!!.favorites.containsKey(uid)){
                    // When the button is clicked -> 좋아요 취소
                    contentDTO?.favoriteCount = contentDTO?.favoriteCount -1
                    contentDTO?.favorites.remove(uid)
                }else{
                    // When the button is not clicked -> 좋아요
                    contentDTO?.favoriteCount = contentDTO?.favoriteCount +1
